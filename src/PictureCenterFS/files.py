...
from Tools.Directories import copyfile, fileExists, pathExists, resolveFilename, SCOPE_CURRENT_SKIN, SCOPE_PLUGINS
...
class PictureCenterFS7_Filemenu(Screen):
	skindatei = f"{PLUGINPATH}/skin/{RESOLUTION}/pcFS_menulist.xml"
	...
	def __init__(self, session, file1, file2=""):
		...
		nums = ["", "1 ", "2 ", "3 "]
		if exists(resolveFilename(SCOPE_CURRENT_SKIN, f"skin_default/buttons/key_{len(pics)}.png")):
			template = "with_numpic"
			for i in range(1, len(pics)):
				pics[i] = LoadPixmap(resolveFilename(SCOPE_CURRENT_SKIN, f"skin_default/buttons/key_{i}.png"))
				nums[i] = ""
		mlist.append((f"{nums[1]}{_('Copy selected/displayed file')}", "copy", pics[1]))
		if len(self.file2) > 0:
			mlist.append((f"{nums[2]}{_('save changes in the original image')}", "save_rotat", pics[2]))
			self.setTitle(f"{PLUGINNAME}: {_('Options for edited image')}")
		else:
			mlist.append((f"{nums[2]}{_('Move selected/displayed file')}", "move", pics[2]))
			mlist.append((f"{nums[3]}{_('Delete selected/displayed file')}", "delete", pics[3]))
		self.m_liste = mlist
		...
		self.setTitle(f"{PLUGINNAME}: {_('Options for selected/displayed file')}")
...
#bearbeiten ############################################
	def save_rotat(self):
		text1 = f"{_('selceted File is:')}\n{self.file1}\n\n"
		self.session.openWithCallback(self.save_rotat2, MessageBox, _(f"{text1}{_('Do you really overwrite this file?')}"), MessageBox.TYPE_YESNO)
	...
	def copy(self):
		...
		self.endung = f".{basename(self.file1).split(".")[-1]}"
		text1 = basename(self.file1).replace(f".{basename(self.file1).split(".")[-1]}", "")
		self.session.openWithCallback(self.copy2, VirtualKeyBoard, title=_("Edit file-save-name:"), text=text1)

	def copy2(self, filename):
		if filename:
			self.oldpath = f"{dirname(self.file1)}/"
			self.filename = f"{filename}{self.endung}"
		else:
			self.filename = basename(self.file1)
		self.session.openWithCallback(self.callCopy, BackupLocationBox, _("Please select target path..."), f"{split(dirname(self.file1))[0]}/", getsize(self.file1))  # "/tmp/")
	...
	def del_file(self):
		text1 = f"{_('selceted File is:')}\n{self.file1}\n\n"
		self.session.openWithCallback(self.del_file_ok, MessageBox, _(f"{text1}{_('Do you really want to delete this file?')}"), MessageBox.TYPE_YESNO)
	...
	def callmove(self, path):
		if path is not None:
			source = self.file1
			target = f"{path}{basename(self.file1)}"
			try:
...
class save_mark:
	def __init__(self, session, name):
		copyfile("/tmp/pcfs_mark", f"/etc/ConfFS/{name}")
...
class backup:
	...
	def callBackup(self, path):
		if self.num and path is not None:
			if pathExists(path):
				if self.num == 3:
					self.settigspath = f"{path}ConfFS_backup.tar.gz"
		...
		if self.num and self.settigspath:
			...
			if pathExists(mpath) is True:
				...
				for file in listdir(mpath):
					files.append(f"/etc/ConfFS/{file}")
				for arg in files:
					tarFiles += f"{arg} "
			try:
				com = f"tar czvf {self.settigspath} {tarFiles}"
	...
	def callRestore(self, path1):
		if self.tarfile and path1:
			self.settigspath = f"{path1}{self.tarfile}"
			if fileExists(self.settigspath):
	...
	def callOverwriteSettings(self, res=None):
		if res:
			try:
				com = f"cd /; tar xzvf {self.settigspath}"
